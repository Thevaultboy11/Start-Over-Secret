  ...
  useEffect(() => {
    ...
    onValue(userRef, (snapshot) => {
      ...
      if (!reflectionData || typeof reflectionData !== 'object') {
        ...
        const reflectionsArray = Object.entries(reflectionData).map(([id, val]: any) => ({ id, ...val })).sort((a, b) => b.date.localeCompare(a.date));
  ...
  return (
    <>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Typography variant="h6" fontWeight="bold">Latest Reflections</Typography>
        <IconButton onClick={() => router.push('/add-recordings')} sx={{ bgcolor: '#FF073A', color: 'white', borderRadius: '50%', p: 1 }}>
          <AddIcon />
        </IconButton>
      </Box>
      ...
      {fireBaseloading ? (
        ...
        <Typography color="white" fontSize={14} textAlign="center">No reflections yet. Start by adding your first!</Typography>
      ) : (
        <>
          <Paper sx={{ bgcolor: '#000', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 2, p: 2, mb: 3 }}>
            <Typography fontSize={16} fontWeight="bold" gutterBottom>
              Mood & Return Likelihood Over Time
            </Typography>
                ...
                <TableBody>
                  {latestReflections.map((entry) => (
                    <TableRow key={entry.id}>
                      <TableCell sx={{ fontSize: 24, color: 'white' }}>{moodEmoji(entry.mood)}</TableCell>
                      <TableCell sx={{ fontSize: 14, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'white' }}>
          ...
          <Dialog open={!!selectedDay} onClose={() => setSelectedDay(null)} fullWidth>
            <Paper sx={{ bgcolor: '#000', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 2, p: 2 }}>
              <DialogTitle sx={{ color: 'white' }}>Reflection - {selectedDay?.date}</DialogTitle>
              <DialogContent>
                <Typography fontSize={14} mb={1} color="white"><strong>Mood:</strong> {selectedDay?.mood}</Typography>
                <Typography fontSize={14} mb={1} color="white"><strong>Return Likelihood:</strong> {selectedDay?.returnLikelihood}</Typography>
                <Typography fontSize={14} mb={1} color="white"><strong>Reflection:</strong> {selectedDay?.reflectionText}</Typography>
                <Typography fontSize={14} color="white"><strong>Activities:</strong> {selectedDay?.activities?.join(', ')}</Typography>
              </DialogContent>
            </Paper>
          </Dialog>
        </>
